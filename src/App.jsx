  ...
  return (
    <div className="min-h-screen bg-stone-50 flex items-center justify-center p-6 font-sans">
      ...
      <div className="cpf-root w-full max-w-md">
        ...
        <div className="flex items-center justify-between bg-white rounded-2xl border border-stone-200 shadow-sm px-5 py-3 mb-4">
          <span className="text-sm font-medium text-stone-500">View</span>
          <div className="flex items-center gap-3">
            <span className={`text-sm font-semibold transition-colors ${!isAnnual ? "text-stone-800" : "text-stone-300"}`}>Monthly</span>
            <button
              onClick={() => setIsAnnual((v) => !v)}
              className={`relative w-11 h-6 rounded-full transition-colors duration-200 ${isAnnual ? "bg-red-600" : "bg-stone-200"}`}
            >
            ...
            <span className={`text-sm font-semibold transition-colors ${isAnnual ? "text-stone-800" : "text-stone-300"}`}>Annual</span>
          </div>
        </div>
        ...
        <div className={`bg-white rounded-2xl border shadow-sm p-6 transition-opacity duration-300 ${hasValue ? "opacity-100 border-stone-200" : "opacity-40 border-stone-100"}`}>
          ...
          <Row
            label="Take-Home Pay"
            sub={`Gross minus employee CPF${isAnnual ? " × 12" : ""}`}
            value={fmt(takeHome * mult)}
            accent
          />
          ...
          <div className="mt-4 pt-4 border-t border-stone-100 grid grid-cols-2 gap-3">
            <div className="bg-stone-50 rounded-xl p-3">
              ...
              <p className="text-sm font-bold text-stone-700 font-mono mt-0.5">{fmt(totalContrib * mult)}</p>
            </div>
            <div className="bg-stone-50 rounded-xl p-3">
              <p className="text-xs text-stone-400 font-medium">Total Employer Cost</p>
              <p className="text-sm font-bold text-stone-700 font-mono mt-0.5">{fmt(totalCost * mult)}</p>
            </div>
          </div>
        </div>
